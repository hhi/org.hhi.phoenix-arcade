...
let active='architecture', activeKind='', selected=null, query='';
const esc=value=>String(value??'').replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[char]));
const norm=value=>JSON.stringify(value).toLowerCase();
const docs=node=>{const documentation=node.evidence?.documentation;return Array.isArray(documentation)?documentation:documentation?[documentation]:[];};
...
function cLink(node){if(!node.source?.path)return '';const source=node.source.path.split('/').pop();const name=source.replace(/\.h$/i,'-h.html').replace(/\.c$/i,'.html');return '<a href="../../context/source/'+esc(name)+'#L'+esc(node.source.line)+'" target="_blank">Open source viewer: '+esc(node.source.path)+':'+esc(node.source.line)+'</a>';}
function docLink(path){const htmlPath=path.replace(/\.md$/i,'.html');const relative=htmlPath.startsWith('c-annotated/')?'../'+htmlPath.slice('c-annotated/'.length):'../../'+htmlPath;return '<a href="'+esc(relative)+'" target="_blank">Open '+esc(htmlPath)+'</a>';}
function asmLinks(node){const ranges=asmRanges(node);if(!ranges.length)return '<p>No Z80 range is mapped to this item.</p>';return ranges.map(range=>`<p class="asm"><a href="../../context/code-annotated.html#asm-${esc(String(range.start).toLowerCase())}" target="_blank">Open annotated Z80: ${esc(range.start)}–${esc(range.end)}</a></p>`).join('');}
const kindLabel=kind=>kind.replaceAll('-',' ').replace(/\b\w/g,letter=>letter.toUpperCase());
function renderNav(){topicNav.innerHTML='<p class="root">Phoenix knowledge base</p>'+TOPICS.map(topic=>{const nodes=topicNodes(topic), kinds=['c-function','game-state','ram-slot','table-asset','asm-routine','rom-pattern'].filter(kind=>nodes.some(node=>node.kind===kind));return `<button class="topic ${active===topic.key&&!query?'active':''}" data-topic="${topic.key}">${esc(topic.title)}<small>${nodes.length} mapped items</small></button><div class="subtopics">${kinds.map(kind=>`<button class="subcategory ${active===topic.key&&activeKind===kind&&!query?'active':''}" data-topic="${topic.key}" data-kind="${kind}">${kindLabel(kind)} (${nodes.filter(node=>node.kind===kind).length})</button>`).join('')}</div>`;}).join('');topicNav.querySelectorAll('.topic').forEach(button=>button.onclick=()=>{active=button.dataset.topic;activeKind='';query='';search.value='';selected=null;render();});topicNav.querySelectorAll('.subcategory').forEach(button=>button.onclick=()=>{active=button.dataset.topic;activeKind=button.dataset.kind;query='';search.value='';selected=null;render();});}
...
function fileGroups(nodes){const groups=new Map();nodes.forEach(node=>{const file=node.source?.path||'No C source file';if(!groups.has(file))groups.set(file,[]);groups.get(file).push(node);});return [...groups.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([file,entries])=>`<h3 class="file-heading">${esc(file)}</h3><div class="cards">${entries.map(card).join('')}</div>`).join('');}
function showTopic(topic){let nodes=query?GRAPH.nodes.filter(node=>node.kind!=='claim'&&norm(node).includes(query)):topicNodes(topic);if(activeKind&&!query)nodes=nodes.filter(node=>node.kind===activeKind);nodes=nodes.sort((a,b)=>a.name.localeCompare(b.name));const count=nodes.length;const title=query?'Search results':activeKind?`${topic.title} · ${kindLabel(activeKind)}`:topic.title;const body=activeKind==='c-function'&&!query?fileGroups(nodes):`<div class="cards">${nodes.map(card).join('')}</div>`;content.innerHTML=`<h2>${esc(title)}</h2><p class="lead">${query?`Everything mapped for “${esc(query)}”.`:esc(topic.description)}</p><div class="summary">${count} mapped item${count===1?'':'s'}. Select an item to see direct links to the C implementation, its original Z80 range and the explanatory documentation.</div>${count?body:'<p class="empty">No mapped items match this selection.</p>'}`;wireCards();}
