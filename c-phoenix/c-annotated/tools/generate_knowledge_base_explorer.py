...
function asmLinks(node){const ranges=asmRanges(node);if(!ranges.length)return '<p>No Z80 range is mapped to this item.</p>';return ranges.map(range=>`<p class="asm"><a href="../../context/Phoenix.asm" target="_blank">Open original Z80: ${esc(range.start)}–${esc(range.end)}</a></p>`).join('');}
