...
def render(source,destination,explorer):
    ...
    # C and header references must use the standalone source viewer, never a
    # raw source file. Preserve an existing line fragment as its stable page
    # anchor.
    def source_page(match):
        c_path, fragment = match.groups()
        source = Path(c_path)
        name = source.stem + ("-h.html" if source.suffix == ".h" else ".html")
    ...
    body=re.sub(r'href="([^"#]+\.(?:c|h))(#[^"]*)?"', source_page, body)
