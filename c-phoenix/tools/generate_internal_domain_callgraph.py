...
def main():
    ...
    for caller, callee in raw_calls:
        if func_to_category[caller] == func_to_category[callee]:
            internal_calls.add((caller, callee))
            active_nodes.add(caller)
            active_nodes.add(callee)

    dot_path = os.path.join(dir_path, "context", "graphs", "internal_domain_callgraph.dot")
    with open(dot_path, "w") as f:
        f.write("digraph InternalDomainGraph {\n")
        ...
        for category in categories.keys():
            ...
            f.write(f"    label=\"{category.replace('_', ' ')} (Internal Cohesion)\";\n")
            f.write("    fontname=\"Helvetica-bold\";\n")
            f.write("    style=\"rounded,filled\";\n")
            f.write("    fillcolor=honeydew;\n")
            for func in nodes_in_cat:
                f.write(f"    \"{func}\";\n")
            f.write("  }\n")

        for caller, callee in internal_calls:
            f.write(f"  \"{caller}\" -> \"{callee}\";\n")

        f.write("}\n")

    try:
        svg_path = os.path.join(dir_path, "context", "graphs", "internal_domain_callgraph.svg")
        subprocess.run(["dot", "-Tsvg", dot_path, "-o", svg_path], check=True)
        print("Internal SVG generated successfully at " + svg_path)
