...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .theme-control {{ display: flex; align-items: center; justify-content: space-between; gap: .5rem; margin: .75rem 0; }} .theme-control span {{ color: var(--muted); font-size: .85rem; }}
    ...
    @media (prefers-color-scheme: light) {{
      :root {{ --bg: #f4f7fb; --panel: #ffffff; --code: #f8fafc; --text: #17212b; --muted: #52606d; --accent: #006e9c; --border: #b7c3cf; --focus: #8a5700; --badge-text: #ffffff; }}
    ...
    body.theme-light {{ color-scheme: light; --bg: #f4f7fb; --panel: #ffffff; --code: #f8fafc; --text: #17212b; --muted: #52606d; --accent: #006e9c; --border: #b7c3cf; --focus: #8a5700; --badge-text: #ffffff; }}
    ...
    body.theme-dark {{ color-scheme: dark; --bg: #101419; --panel: #182027; --code: #0c1116; --text: #d8e0e8; --muted: #92a2b2; --accent: #77d7ff; --border: #2d3a44; --focus: #ffd166; --badge-text: #101419; }}
    body.theme-dark #label-list a:hover, body.theme-dark #label-list a:focus {{ background: #263b49; color: var(--accent); }} body.theme-dark .label-kind-code {{ --kind-color: #ff9e64; color: var(--kind-color); }} body.theme-dark .label-kind-data {{ --kind-color: #7ee787; color: var(--kind-color); }} body.theme-dark .label-kind-unknown {{ --kind-color: #92a2b2; color: var(--kind-color); }}
    body.theme-dark .mnemonic {{ color: #6cb6ff; }} body.theme-dark .mnemonic-emphasis {{ color: #56d4ff; }} body.theme-dark .register {{ color: #c099ff; }} body.theme-dark .directive {{ color: #7dcfff; }} body.theme-dark .literal {{ color: #9ece6a; }} body.theme-dark .comment {{ color: #9aa8b7; }}
    body.theme-dark .control-separator {{ background: color-mix(in srgb, var(--border) 75%, transparent); }} body.theme-dark .label-ref {{ color: #ffe08a; }} body.theme-dark .label-ref-data {{ color: #7ee787; text-decoration-color: #7ee787; }} body.theme-dark .label-ref-unknown {{ color: var(--muted); }} body.theme-dark .symbol-ref, body.theme-dark .c-source-link {{ color: #7ee787; }} body.theme-dark .data-c-representation {{ border-left-color: #7ee787; background: color-mix(in srgb, #7ee787 9%, var(--panel)); }} body.theme-dark #code-tooltip {{ background: #26343d; }} body.theme-dark .callout {{ background: #162b36; }} body.theme-dark .c-function-boundary {{ background: #132e3c; }} body.theme-dark .c-line-highlight {{ background: #4b3f1a; }}
...
<body>
  <div class="layout">
    <aside id="navigator" aria-label="Assembly label navigation">
      ...
      <div class="theme-control"><span>Color theme</span><button id="theme-toggle" type="button" aria-pressed="false">Use light theme</button></div>
      <h2>Navigate</h2>
  ...
  <script>
    ...
    const storedTheme = (() => {{ try {{ return localStorage.getItem('phoenix-asm-theme'); }} catch {{ return null; }} }})();
    const systemTheme = () => matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark';
    const activeTheme = () => document.body.classList.contains('theme-light') ? 'light' : document.body.classList.contains('theme-dark') ? 'dark' : systemTheme();
    const applyTheme = (theme, persist = false) => {{ document.body.classList.remove('theme-light', 'theme-dark'); document.body.classList.add(`theme-${{theme}}`); themeToggle.textContent = `Use ${{theme === 'light' ? 'dark' : 'light'}} theme`; themeToggle.setAttribute('aria-pressed', String(theme === 'dark')); if (persist) {{ try {{ localStorage.setItem('phoenix-asm-theme', theme); }} catch {{ /* Storage can be unavailable. */ }} const url = new URL(location.href); url.searchParams.set('theme', theme); history.replaceState(null, '', url); }} }};
    if (queryTheme === 'light' || queryTheme === 'dark') applyTheme(queryTheme); else if (storedTheme === 'light' || storedTheme === 'dark') applyTheme(storedTheme);
    else {{ themeToggle.textContent = `Use ${{systemTheme() === 'light' ? 'dark' : 'light'}} theme`; themeToggle.setAttribute('aria-pressed', String(systemTheme() === 'dark')); }}
    themeToggle.addEventListener('click', () => applyTheme(activeTheme() === 'light' ? 'dark' : 'light', true));
