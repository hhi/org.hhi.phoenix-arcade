...
header {{ padding:16px 22px; background:#19324a; color:white }} header h1 {{ margin:0; font-size:19px }} header p {{ margin:3px 0 0; color:#cfdeea }}
main {{ display:grid; grid-template-columns:minmax(265px, 28%) 1fr; min-height:calc(100vh - 82px); gap:1px; background:var(--line) }}
aside,section {{ background:white; min-width:0 }} aside {{ padding:16px; overflow:auto }} section {{ padding:18px 22px; overflow:auto }}
...
#tree button:hover {{ background:#edf5f8 }} #tree button.selected {{ background:var(--selected); color:#075a83; font-weight:650 }}
...
function index(node, parent=null) {{ nodes.set(node.id,node); if(parent) parents.set(node.id,parent.id); node.children.forEach(c=>index(c,node)); }} index(DATA.root);
...
function aggregate() {{ const scope=functions(selected), node=nodes.get(selected); let level=node.kind==='domain'?'subsystem':node.kind==='subsystem'?'module':'function';
  ...
  for(const edge of DATA.edges) {{ if(!scope.has(edge.caller)&&!scope.has(edge.callee)) continue; const a=member.get(edge.caller)||'context:'+edge.caller, b=member.get(edge.callee)||'context:'+edge.callee; if(a===b) continue; const key=a+'|'+b; result.set(key,{{a,b,count:(result.get(key)?.count||0)+edge.count}}); }}
