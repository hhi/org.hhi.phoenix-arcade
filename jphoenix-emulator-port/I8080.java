/**
 * A classe i8080 emula o processador Intel 8080. A interface foi baseada na
 * proposta por Adam Davidson e Andrew Pollard para o Jasper - Java Spectrum
 * Emulator.
 */
...
public class I8080 extends Object {
	...
	private int CyclesPerInterrupt;
	private int Cycles;
	...
	private static final int[] OPCODE_CYCLES = {
			...
			4, 4, 4, 4, 4, 4, 7, 4, 4, 4, 4, 4, 4, 4, 7, 4,
			4, 4, 4, 4, 4, 4, 7, 4, 4, 4, 4, 4, 4, 4, 7, 4,
			11, 10, 10, 10, 17, 11, 7, 11, 11, 10, 10, 4, 17, 17, 7, 11,
			11, 10, 10, 10, 17, 11, 7, 11, 11, 10, 10, 4, 17, 4, 7, 11,
			11, 10, 10, 18, 17, 11, 7, 11, 11, 5, 10, 5, 17, 4, 7, 11,
			11, 10, 10, 4, 17, 11, 7, 11, 11, 5, 10, 4, 17, 4, 7, 11
	};

	protected final int cyclesPerInterrupt() {
		return CyclesPerInterrupt;
	}

	protected final int cycleInInterruptFrame() {
		int cycle = Cycles + CyclesPerInterrupt;
		if (cycle < 0) {
			return 0;
		}
		if (cycle >= CyclesPerInterrupt) {
			return CyclesPerInterrupt - 1;
		}
		return cycle;
	}
	// Valores dos flags
	...
	// Mantemos uma lookup table para os valores de paridade
	private static final boolean parity[] = new boolean[256];
	static {
		for (int i = 0; i < 256; i++) {
			boolean p = true;
			for (int j = 0; j < 8; j++) {
				if ((i & (1 << j)) != 0) { // O and só é verdadeiro quando
											// temos um bit 1
					p = !p; // A cada bit 1 encontrado a paridade é atualizada.
				}
			}
			parity[i] = p;
		}
	...
	/**
	 * Registradores O acesso aos valores é efetuado pelos mó todos da classe.
	 */
	...
	/**
	 * Registradores alternativos AF é o registrador A + Flags.
	 */
	...
	/**
	 * Métodos de acesso aos registradores de 16 bits Ambos apresentam duas
	 * interfaces: Se chamados sem parâmetros, retornam o valor atual; Se um
	 * dado parâmertro é passado, este é armazenado no registrador.
	 *
	 */
	...
	/**
	 * Métodos de acesso aos registradores de 8 bits Ambos apresentam duas
	 * interfaces: Se chamados sem parâmetros, retornam o valor atual; Se um
	 * dado parâmertro é passado, este é armazenado no registrador.
	 *
	 *
	...
	/**
	 * Acesso individual aos Flags set* seta o flag e *set retorna o valor
	 * corrente.
	 */
	...
	/** Chamada de procedimento (empilha e desempilha o PC) */
	...
	/** Acesso ao programa - Retorna o byte apontado pelo PC */
	...
	/**
	 * Acesso ao programa - Retorna o word apontado pelo PC. � usado para
	 * leitura de operandos de 16 bits.
	 */
	...
	/**
	 * Portos de I/O Uma classe que implemente o hardware fora do chip deve
	 * sobrecarregar estes m�todos
	 */
	...
	public final void execute() {
		synchronized (this) {
			if (executionThread != null) {
				throw new IllegalStateException("CPU is already executing");
			}
		...
		while (!stopRequested) {
			...
			case 118: /* HALT */
			{
				 ...
				 * local_tstates += (haltsToInterrupt*4); REFRESH(
				 * haltsToInterrupt-1 );
				 */
				break;
			}
	...
	/** Called once for every opcode fetch with the PC before the fetch. */
	protected void recordInstruction(int pc) {
	}

	/** Called when an executed 8080 CALL transfers control to its target. */
	protected void recordCall(int caller, int callee) {
	}

	/** Called when the fetch/execute loop exits. */
	...
	/** Add with carry */
	public final int adc16(int a, int b) {
		int c = Cset() ? 1 : 0;
		int lans = a + b + c;
		int ans = lans & 0xffff;
	...
	/** Add with carry */
	public final int sbc16(int a, int b) {
		int c = Cset() ? 1 : 0;
		int lans = a - b - c;
		int ans = lans & 0xffff;
	...
	public static final int inc16(int a) {
		return (a + 1) & 0xffff;
	}

	public static final int qinc8(int a) {
		return (a + 1) & 0xff;
	}

	/** Quick Decrement : no flags */
	public static final int dec16(int a) {
		return (a - 1) & 0xffff;
	}

	public static final int qdec8(int a) {
		return (a - 1) & 0xff;
	}

	/** Bit toggling */
	public static final int res(int bit, int val) {
		return val & ~bit;
	}

	public static final int set(int bit, int val) {
		return val | bit;
	}
	...
	final boolean isExecutionThread() {
		return executionThread == Thread.currentThread();
	}

	final void writeCpuState(java.io.DataOutput output) throws java.io.IOException {
		output.writeInt(CyclesPerInterrupt);
		output.writeInt(Cycles);
		output.writeInt(_A);
		output.writeInt(_HL);
		output.writeInt(_B);
		output.writeInt(_C);
		output.writeInt(_DE);
		output.writeByte(F());
		output.writeInt(_AF_);
		output.writeInt(_HL_);
		output.writeInt(_BC_);
		output.writeInt(_DE_);
		output.writeBoolean(interruptEnabled);
		output.writeInt(_SP);
		output.writeInt(_PC);
		for (int value : mem) {
	...
	final void readCpuState(java.io.DataInput input) throws java.io.IOException {
		int savedCyclesPerInterrupt = input.readInt();
		if (savedCyclesPerInterrupt != CyclesPerInterrupt) {
			throw new java.io.IOException(
					"CPU clock mismatch: expected " + CyclesPerInterrupt
							+ ", got " + savedCyclesPerInterrupt);
		}
		...
		_C = input.readInt();
		_DE = input.readInt();
		F(input.readUnsignedByte());
		_AF_ = input.readInt();
		_HL_ = input.readInt();
		_BC_ = input.readInt();
		_DE_ = input.readInt();
		interruptEnabled = input.readBoolean();
		_SP = input.readInt();
		_PC = input.readInt();
		for (int i = 0; i < mem.length; i++) {
			mem[i] = input.readUnsignedByte();
		}
	}

	/** Requests that the fetch/execute loop return at the next instruction boundary. */
