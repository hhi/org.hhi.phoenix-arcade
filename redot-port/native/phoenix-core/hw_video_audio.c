...
void wait_vblank_coin(void) {
    // Redot owns the 60 Hz clock and calls phoenix_redot_step() once per
    // physics tick.  The SDL semaphore handshake from the desktop frontend
    // is intentionally absent in this embedded platform.
...
/* One complete original video-frame update, driven by Redot at 60 Hz. */
