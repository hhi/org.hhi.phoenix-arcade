.PHONY: help c-build c2-build c2-run j-build c-test j-test c2-test verify links large-files public-audit romcheck rombuild romprepare gen-rom-data gen-phoenix-tables
...
help:
	...
	@echo "  make gen-rom-data       Regenerate c-phoenix/rom_data.c from ROM_OUTPUT_DIR (aborts on mismatch)"
...
romprepare: rombuild gen-rom-data gen-phoenix-tables
...
gen-rom-data: rombuild
	python3 tools/generate_rom_data.py \
		--program $(ROM_OUTPUT_DIR)/program.rom \
		--graphics $(ROM_OUTPUT_DIR)/graphics.rom \
		--proms $(ROM_OUTPUT_DIR)/proms.rom \
		--existing c-phoenix/rom_data.c \
